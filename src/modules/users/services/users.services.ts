...
import { userLoginRepo, userRegisterRepo } from "../repo/users.repo";
import { comparePassword, hashPassword } from "../utils/bycript";
import { generateToken } from "../utils/jwt";

const userRegisterService = async (userData: IUser) : Promise<{user: IUser}> => {
...
const userLoginService = async (email: string, password: string): Promise<{user: IUser, token: string}> => {
    if (!email || !password) {
